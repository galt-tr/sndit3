...
function Dashboard() {
  ...
  const loadInvoices = async () => {
    try {
      ...
      console.log('Fetched invoices:', fetchedInvoices); // Debug log
      if (Array.isArray(fetchedInvoices)) {
        setInvoices(fetchedInvoices.filter(invoice => invoice && invoice.id && invoice.customer));
      } else {
        console.error('Fetched invoices is not an array:', fetchedInvoices);
        setInvoices([]);
      }
      setError(null);
    } catch (error) {
